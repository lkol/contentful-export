...
function getFullSourceSpace({
  ...
  return new _listr2.default([{
    ...
    task: (0, _listr3.wrapTask)((ctx, task) => {
      ...
      return pagedGet({ source, method: 'getEntries', query: queryEntries }).then(extractItems).then(items => filterDrafts(items, includeDrafts, cdaClient)).then(items => filterArchived(items, includeArchived)).then(items => {
    ...
    task: (0, _listr3.wrapTask)((ctx, task) => {
      ...
      queryAssets.locale = '*';
      return pagedGet({ source, method: 'getAssets', query: queryAssets }).then(extractItems).then(items => filterDrafts(items, includeDrafts, cdaClient)).then(items => filterArchived(items, includeArchived)).then(items => {
